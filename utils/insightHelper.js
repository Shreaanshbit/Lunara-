...
} = require('./cyclePrediction')

exports.getCycleSymptomInsightsInternal = async userId => {
  const cycles = await Cycle.find({ user: userId }).sort({
    periodStart: -1
  })
  ...
  const currentPhase = getCyclePhase(latest.periodStart, avgCycleLength)
